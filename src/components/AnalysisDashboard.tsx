...
export const AnalysisDashboard: React.FC<AnalysisDashboardProps> = ({
  ...
  const copyAllImprovedBullets = () => {
    ...
    copyToClipboard(
      ...
      "All improved bullet recommendations",
    );
  };
  ...
  const handleCustomRewrite = async () => {
    if (!customBulletInput.trim() || customBulletInput.length < 5) {
      onShowToast(
        "Input Needed",
        "Please enter a bullet point to rewrite.",
        "warning",
      );
      return;
    }
    ...
    try {
      ...
      onShowToast(
        "Rewrite Generated",
        "Produced 3 high-impact versions.",
        "success",
      );
    } catch (err: any) {
      onShowToast(
        "Rewrite Error",
        err.message || "Failed to rewrite bullet.",
        "error",
      );
    } finally {
  ...
  const categoryScoreList = [
    {
      ...
      score: analysis.categoryScores?.keywordOptimization ?? 75,
    },
    {
      key: "skillsMatch",
      label: "Skills Alignment",
      score: analysis.categoryScores?.skillsMatch ?? 80,
    },
    {
      key: "experienceImpact",
      label: "Experience & Impact",
      score: analysis.categoryScores?.experienceImpact ?? 70,
    },
    {
      key: "educationRelevance",
      label: "Education & Credentials",
      score: analysis.categoryScores?.educationRelevance ?? 85,
    },
    {
      key: "formattingAndLayout",
      label: "Formatting & Layout",
      score: analysis.categoryScores?.formattingAndLayout ?? 88,
    },
    {
      key: "resumeStructure",
      label: "Resume Structure",
      score: analysis.categoryScores?.resumeStructure ?? 80,
    },
    {
      key: "quantifiableMetrics",
      label: "Quantifiable Metrics",
      score: analysis.categoryScores?.quantifiableMetrics ?? 60,
    },
    {
      key: "actionVerbsAndTone",
      label: "Action Verbs & Tone",
      score: analysis.categoryScores?.actionVerbsAndTone ?? 78,
    },
  ];

  return (
    <div
      id="analysis-dashboard-page"
    ...
    >
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-md p-6 sm:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            <span className="px-2.5 py-0.5 text-xs font-bold uppercase tracking-wider rounded-md bg-indigo-100 text-indigo-700 border border-indigo-200 ">
              Analysis Report
            </span>
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-1">
            {analysis.targetRole
              ...
              : "General ATS Recruiter Compatibility Scan"}
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2.5 w-full md:w-auto">
          <button
            onClick={handlePrint}
            className="px-3.5 py-2.5 text-xs font-semibold rounded-xl border border-slate-200 bg-slate-50 text-slate-700 hover:bg-slate-100 :bg-slate-700 flex items-center gap-1.5 transition-colors"
          ...
          <button
            ...
            className="px-3.5 py-2.5 text-xs font-semibold rounded-xl border border-slate-200 bg-slate-50 text-slate-700 hover:bg-slate-100 flex items-center gap-1.5 transition-colors"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
        ...
        <div className="lg:col-span-4 bg-white rounded-2xl border border-slate-200 p-6 shadow-md flex flex-col justify-between items-center text-center">
          <div className="w-full flex justify-between items-center pb-2 border-b border-slate-100 ">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-500">
              Overall ATS Score
            </span>
            <span
              className={`px-2.5 py-0.5 text-xs font-bold rounded-full ${scoreColors.badge}`}
            >
              {analysis.scoreTier}
            </span>
          </div>
          ...
          <div className="w-full pt-4 border-t border-slate-100 flex flex-col gap-3 text-left">
            <div className="p-2.5 rounded-lg bg-slate-50 border border-slate-100 ">
              ...
              <span className="text-sm font-extrabold text-slate-900 ">
                {analysis.experienceInsight?.measurableResultsCount ?? 3}{" "}
                Detected
              </span>
            </div>
            <div className="p-2.5 rounded-lg bg-slate-50 border border-slate-100 ">
        ...
        <div className="lg:col-span-8 bg-white rounded-2xl border border-slate-200 p-6 shadow-md flex flex-col justify-between">
          <div>
            ...
            <button
              ...
              className={`flex items-center gap-2 py-3 px-3 sm:px-4 text-xs sm:text-sm font-bold border-b-2 transition-all whitespace-nowrap ${
                isActive
                  ...
                  : "border-transparent text-slate-500 hover:text-slate-800 :text-slate-200"
              }`}
            >
      ...
      {activeTab === "overview" && (
        <div className="flex flex-col lg:grid lg:grid-cols-2 gap-6">
          {/* Strengths */}
          <div className="bg-white rounded-2xl border border-emerald-200 p-6 shadow-md space-y-4">
            <div className="flex items-center gap-2.5 pb-3 border-b border-emerald-100 ">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Elements working in your favor with ATS & recruiters
                </p>
              </div>
            </div>
            ...
            <ul className="space-y-3">
              {analysis.strengths.map((str, idx) => (
                <li
                  key={idx}
                  className="p-3 rounded-xl bg-emerald-50/50 border border-emerald-100 text-xs text-slate-700 flex items-start gap-2.5 leading-relaxed"
                >
          ...
          {/* Weaknesses */}
          <div className="bg-white rounded-2xl border border-rose-200 p-6 shadow-md space-y-4">
            <div className="flex items-center gap-2.5 pb-3 border-b border-rose-100 ">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Fix these to prevent ATS auto-rejection
                </p>
              </div>
            </div>
            ...
            <ul className="space-y-3">
              {analysis.weaknesses.map((weak, idx) => (
                <li
                  key={idx}
                  className="p-3 rounded-xl bg-rose-50/50 border border-rose-100 text-xs text-slate-700 flex items-start gap-2.5 leading-relaxed"
                >
      ...
      {activeTab === "keywords" && (
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-amber-200 p-6 shadow-md space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-amber-100 ">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base font-bold text-slate-900 ">
                  High-Priority Missing ATS Keywords (
            ...
            <p className="text-xs text-slate-600 ">
              Recruiters and automated filters scan for these industry terms. If
              you have experience with any of these, weave them naturally into
              your Summary, Experience, or Skills sections.
            </p>
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              {analysis.missingKeywords?.map((kw, i) => (
                <button
                  key={i}
                  onClick={() =>
                    copyToClipboard(kw, `kw-${i}`, `Keyword "${kw}"`)
                  }
                  className="px-3 py-1.5 text-xs font-semibold rounded-lg bg-amber-50 border border-amber-200 text-amber-900 hover:bg-amber-100 hover:border-amber-300 transition-all flex items-center gap-1.5 group"
                >
                  <span>+ {kw}</span>
                  {copiedId === `kw-${i}` ? (
                ...
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {analysis.detectedSkills?.technical?.length ? (
                    analysis.detectedSkills.technical.map((s, i) => (
                      <span
                        key={i}
                        className="px-2 py-0.5 text-[11px] font-medium rounded bg-indigo-50 text-indigo-700 border border-indigo-100 "
                      >
                        {s}
                      </span>
                    ))
                ...
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {analysis.detectedSkills?.programmingLanguages?.length ? (
                    analysis.detectedSkills.programmingLanguages.map((s, i) => (
                      <span
                        key={i}
                        className="px-2 py-0.5 text-[11px] font-medium rounded bg-sky-50 text-sky-700 border border-sky-100 "
                      >
                        {s}
                      </span>
                    ))
                ...
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {analysis.detectedSkills?.tools?.length ? (
                    analysis.detectedSkills.tools.map((s, i) => (
                      <span
                        key={i}
                        className="px-2 py-0.5 text-[11px] font-medium rounded bg-purple-50 text-purple-700 border border-purple-100 "
                      >
                        {s}
                      </span>
                    ))
                ...
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {analysis.detectedSkills?.soft?.length ? (
                    analysis.detectedSkills.soft.map((s, i) => (
                      <span
                        key={i}
                        className="px-2 py-0.5 text-[11px] font-medium rounded bg-emerald-50 text-emerald-700 border border-emerald-100 "
                      >
                        {s}
                      </span>
                    ))
      ...
      {activeTab === "sections" && (
        <div className="space-y-4">
          <div className="p-4 rounded-xl bg-slate-100 text-xs text-slate-600 flex items-center justify-between">
            <span>
              Click any section card below to view strengths, flagged
              weaknesses, and targeted advice.
            </span>
            ...
            return (
              ...
              >
                ...
                <div
                  ...
                  className="p-5 flex items-center justify-between cursor-pointer hover:bg-slate-50/80 :bg-slate-800/40 select-none transition-colors"
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-9 h-9 rounded-xl flex items-center justify-center font-bold text-xs ${colors.badge}`}
                    >
                      {sec.score}
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-3">
                    <span
                      className={`px-2.5 py-0.5 text-xs font-bold rounded-full ${colors.badge}`}
                    >
                      {sec.score}/100
                    </span>
                    {isOpen ? (
                    ...
                    <div className="p-3.5 rounded-xl bg-indigo-50/60 border border-indigo-100 space-y-2">
                      ...
                      <ul className="space-y-1.5 text-slate-700 ">
                        {sec.suggestions?.map((sugg, i) => (
                          <li
                            key={i}
                            className="flex items-start justify-between gap-2 p-2 rounded-lg hover:bg-white/60 :bg-slate-900/40 transition-colors"
                          >
                            ...
                            <button
                              ...
                              onClick={() =>
                                copyToClipboard(
                                  sugg,
                                  `sugg-${sec.sectionName}-${i}`,
                                  "Recommendation",
                                )
                              }
                              title="Copy suggestion"
                              className="shrink-0 px-2 py-1 text-[11px] font-semibold rounded-md border border-slate-200 hover:bg-white text-slate-600 flex items-center gap-1 cursor-pointer transition-colors"
                            >
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                <h3 className="text-lg font-bold text-slate-900 ">
                  Achievement-Oriented Bullet Transformations (
                  {analysis.bulletPointImprovements?.length || 0})
                </h3>
                <p className="text-xs text-slate-500">
                  Converted from passive duties into measurable STAR/Google XYZ
                  achievements with quantifiable metrics.
                </p>
              </div>

              {analysis.bulletPointImprovements &&
                analysis.bulletPointImprovements.length > 0 && (
                  <button
                    id="copy-all-improved-bullets-btn"
                    onClick={copyAllImprovedBullets}
                    className="self-start sm:self-auto px-3.5 py-2 text-xs font-bold rounded-xl border border-indigo-200 bg-indigo-50 hover:bg-indigo-100 :bg-indigo-900/60 text-indigo-700 flex items-center gap-2 shadow-xs transition-colors cursor-pointer"
                  >
            ...
            <div className="grid grid-cols-1 gap-4">
              {analysis.bulletPointImprovements?.map((item) => (
                <div
                  ...
                  className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm space-y-3 transition-all hover:shadow-md"
                >
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-bold uppercase px-2 py-0.5 rounded bg-indigo-100 text-indigo-700 border border-indigo-200 ">
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 text-xs font-bold rounded-lg border border-slate-200 hover:bg-slate-100 :bg-slate-800 flex items-center gap-1.5 text-slate-700 transition-all cursor-pointer shadow-xs"
                    >
                      {copiedId === item.id ? (
                        <>
                  ...
                  <div className="flex flex-col lg:grid lg:grid-cols-2 gap-4 text-xs">
                    {/* Weak Original */}
                    <div className="p-4 rounded-xl bg-rose-50/40 border border-rose-100 space-y-2">
                      ...
                      <p className="text-slate-700 font-mono text-[11px] leading-relaxed bg-white/60 p-2.5 rounded-lg border border-rose-100/80 ">
                        "{item.original}"
                      </p>
                    ...
                    {/* AI Enhanced */}
                    <div className="p-4 rounded-xl bg-emerald-50/50 border border-emerald-200 space-y-2 relative group">
                      <div className="flex items-center justify-between">
                      ...
                      <div className="p-2.5 rounded-lg bg-white/80 border border-emerald-200/80 text-slate-900 font-medium text-xs leading-relaxed select-all">
                        "{item.improved}"
                      </div>

                      {item.metricAddedSuggestion && (
                        <div className="text-emerald-700 text-[11px] pt-1">
                          <strong>Metric Strategy:</strong>{" "}
                          {item.metricAddedSuggestion}
                        </div>
                      )}
                    </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-slate-300">
                  Type or paste any single duty or bullet from your resume to
                  instantly generate 3 high-impact versions.
                </p>
              </div>
            </div>
            ...
            {customRewriteResult && (
              <div className="mt-4 pt-4 border-t border-slate-800 space-y-3">
                <div className="p-3 rounded-lg bg-indigo-950/60 border border-indigo-800/80 text-xs text-indigo-200">
                  <strong>Recruiter Critique:</strong>{" "}
                  {customRewriteResult.critique}
                </div>

                <div className="flex flex-col lg:grid lg:grid-cols-3 gap-3 text-xs">
                  {/* 1. Metrics-Focused */}
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <button
                          ...
                          onClick={() =>
                            copyToClipboard(
                              customRewriteResult.metricsFocused,
                              "custom-m",
                              "Metrics version",
                            )
                          }
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <button
                          ...
                          onClick={() =>
                            copyToClipboard(
                              customRewriteResult.actionOriented,
                              "custom-a",
                              "Action version",
                            )
                          }
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <button
                          ...
                          onClick={() =>
                            copyToClipboard(
                              customRewriteResult.atsOptimized,
                              "custom-ats",
                              "ATS version",
                            )
                          }
            ...
            <div className="flex flex-col lg:grid lg:grid-cols-2 gap-4">
              {analysis.formattingIssues?.length ? (
                ...
                <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200 text-xs text-emerald-800 ">
                  ✅ Clean formatting detected. Standard section headers and
                  linear layouts verified.
                </div>
              )}
            </div>
          </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-md space-y-3">
              ...
              <div className="pt-2 text-xs space-y-1 text-slate-500">
                <div>
                  ...
                  {analysis.experienceInsight?.jobTitlesDetected?.join(", ") ||
                    "Standard Roles"}
                </div>
                <div>
                  <strong>Estimated Total Experience:</strong>{" "}
                  {analysis.experienceInsight?.estimatedYearsExperience ||
                    "N/A"}
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-md space-y-3">
              ...
              <div className="pt-2 text-xs space-y-1 text-slate-500">
                <div>
                  ...
                  {analysis.educationInsight?.degreeDetected ||
                    "Degree detected"}
                </div>
                <div>
                  <strong>Institution:</strong>{" "}
                  {analysis.educationInsight?.institutionDetected ||
                    "Institution noted"}
                </div>
              </div>
            </div>
