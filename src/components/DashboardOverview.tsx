...
interface DashboardOverviewProps {
  ...
  onNavigate: (view: "upload" | "jobmatch" | "history" | "analysis") => void;
...
export const DashboardOverview: React.FC<DashboardOverviewProps> = ({
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/analyses?userId=${user?.id || "demo-user-123"}`);
  ...
  const scoreDiff = latestAnalysis && previousAnalysis ? latestAnalysis.atsScore - previousAnalysis.atsScore : null;

  return (
    <div id="dashboard-overview-page" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      {/* Welcome Banner */}
      <div className="bg-gradient-to-r from-indigo-900 via-indigo-800 to-slate-900 text-white rounded-3xl p-6 sm:p-10 shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-2xl space-y-3">
          ...
          <p className="text-xs sm:text-sm text-indigo-200 leading-relaxed">
            Upload your latest resume draft or test against a target job description to pinpoint missing keywords and optimize your hiring pipeline.
          </p>
          ...
          <div className="pt-3 flex flex-wrap gap-3">
            ...
            <button
              ...
              className="px-5 py-3 rounded-xl bg-white/10 hover:bg-white/20 text-white text-xs sm:text-sm font-semibold flex items-center gap-2 transition-colors cursor-pointer"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm flex items-center justify-between">
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-1 truncate max-w-[180px]">
              {latestAnalysis ? latestAnalysis.filename : "No resume analyzed yet"}
            </p>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm flex items-center justify-between">
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-1">
              {analyses.length > 0 ? "Saved in your private history" : "Ready for your first upload"}
            </p>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm flex items-center justify-between">
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-1">
              {latestAnalysis && latestAnalysis.atsScore >= 80 ? "Top 10% Applicant Pool" : "Optimization Recommended"}
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-md space-y-4">
          ...
          {loading ? (
            <div className="py-12 text-center text-xs text-slate-400">Loading recent scans...</div>
          ) : analyses.length === 0 ? (
            <div className="py-10 text-center space-y-3">
              <p className="text-xs text-slate-500">
                You have not analyzed any resumes yet. Start by uploading a PDF or choosing a sample.
              </p>
              <button
                onClick={() => onNavigate("upload")}
            ...
            <div className="space-y-3">
              {analyses.slice(0, 4).map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-950/40 hover:bg-slate-100/80 transition-colors flex items-center justify-between gap-4"
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-xs ${colors.badge}`}>
                        {item.atsScore}
                      </div>
                      <div>
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 text-xs font-bold rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:text-indigo-600 flex items-center gap-1.5"
                    >
            ...
            <div className="space-y-2.5">
              {SAMPLE_RESUMES.map((sample) => (
                <div
                  ...
                  className="p-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 hover:border-indigo-400 hover:bg-indigo-50/40 cursor-pointer transition-all space-y-1 group"
                >
                  <div className="flex items-center justify-between text-xs font-bold text-slate-800 dark:text-slate-200 group-hover:text-indigo-600">
                    ...
                    <span className="text-[10px] uppercase font-bold text-indigo-600">{sample.experienceLevel}</span>
                  </div>
